...
这是一个非常精准的 “产品打磨” 需求。我们不改动任何核心逻辑（Exa/Gemini/Streamlit Secrets），只对 UI 交互 和 文档体验 进行升维。

🛠️ 改动详情 (V2.6 Final Polish):
📱 手机端侧边栏“复活” (Critical Fix):

问题根源： 原代码里的 header { visibility: hidden; } 把手机端左上角的“汉堡菜单”按钮也给藏起来了，导致无法打开侧边栏。

修复： 我删除了这一行，改为只隐藏右上角的菜单和装饰条，保留 Header 区域的点击能力，确保手机上能点开侧边栏看 Live Feed。

📘 Manual (手册) 下沉与升维:

位置： 从顶部标题旁移到了 页面最底部，使用 st.expander (折叠栏)。平时它是收起的，完全不干扰主界面操作（不喧宾夺主）。

内容： 增加了 中英文切换 (Radio Button)。文案升级为 “Operational Protocol (操作协议)”，使用金融/技术术语（如 RAG、Semantic Layer、Bayesian Inference）。

致谢： 在手册底部增加了一个 霓虹风格的 Exa.ai 致谢卡片，强调其作为 Core Engine 的地位。

UI 细节：

增加了 @media 查询，在手机上自动调小标题字号，防止换行难看。

🚀 Be Holmes V2.6 (Mobile Ready & Pro Manual)
请全选复制覆盖 streamlit_app.py：
